...
namespace Unity.Multiplayer.Playmode.Workflow.Editor
{
    class MainPlayerSystems
    {
        ...
        internal void Listen(WorkflowMainEditorContext mppmContext, MainEditorContext vpContext)
        {
            ...
            {
                var hasPlayer = VirtualProjectWorkflow.WorkflowMainEditorContext.SystemDataStore.TryLoadPlayerJson((int)message.PlayerIndex, out var player);
                Debug.Assert(hasPlayer, $"Unknown Player with index {message.PlayerIndex}");
            ...
            EditorApplication.delayCall += () =>
            {
                ...
                var storedEditorVersion = mppmContext.SystemDataStore.GetEditorVersion();
                if (storedEditorVersion != editorVersion)
                {
                    ApplicationEvents.InvokeEditorVersionChanged(storedEditorVersion, editorVersion);
                    mppmContext.SystemDataStore.UpdateEditorVersion(editorVersion);
                }
            };
