    ...
    class MessagingService
    {
        ...
        public void HandleUpdate()
        {
            ...
            for (var i = m_Acks.Count - 1; i >= 0; i--)
            {
                ...
                {
                    ack.ErrorCallback?.Invoke($"Timeout [{(DateTime.UtcNow - ack.SentTimestamp).TotalSeconds}>{k_MessageTimeoutSeconds}] reached for client message '{ack.EventType}:{messageId}'");
                    m_Acks.RemoveAt(i);
                }
            }
        }
...
#if UNITY_MP_TOOLS_DEV_LOGMESSAGES
                MppmLog.Debug($"Message Sent: {message.GetType().Name}\nPayload: {JsonConvert.SerializeObject(message)}");
#endif
            }
        }
        ...
        void HandleIncomingData(byte[] bytes)
        {
            ...
            var message = SerializeMessageMapping.SerializeMessageDelegatesMap[type].DeserializeFunc(messageReader);
